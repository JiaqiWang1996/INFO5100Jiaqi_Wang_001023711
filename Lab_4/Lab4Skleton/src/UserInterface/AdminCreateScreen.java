...
public class AdminCreateScreen extends javax.swing.JPanel {
    ...
    private void btnCreateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCreateActionPerformed
        ...
        if( !usernamePatternCorrect()){
         JOptionPane.showMessageDialog(null, "Your username is invalid ,please try again","Warning",JOptionPane.WARNING_MESSAGE);
         txtUser.setBorder(BorderFactory.createLineBorder(Color.red));
        }
        else if(txtPword.getText().length()<6||! passwordPatternCorrect()){
         JOptionPane.showMessageDialog(null, "Your password is invalid ,please try again","Warning",JOptionPane.WARNING_MESSAGE);
        }
        else if(!(txtPword.getText()).equals(txtRePword.getText())){
         JOptionPane.showMessageDialog(null, "Two password entries are inconsistent","Warning",JOptionPane.WARNING_MESSAGE);
        }
        else{
        if(radioCustomer.isSelected()){
        Customer user=admin.custDir.addCustomer(txtPword.getText(), txtUser.getText());
        JOptionPane.showMessageDialog(null, "Create the customer account successfully");
        ...
        Supplier user=admin.suppDir.addSupplier(txtPword.getText(), txtUser.getText());
         JOptionPane.showMessageDialog(null, "Create the supplier account successfully");
    ...
    private boolean passwordPatternCorrect() {
       //To change body of generated methods, choose Tools | Templates.
        Pattern p=Pattern.compile("^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$");//6位以上，含大小写特殊字符
